...
import {
  ...
  PanelRightOpen, Workflow, Globe, ClipboardList, UsersRound, X
} from 'lucide-react'
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="fixed right-0 top-0 h-screen z-40 hidden lg:flex flex-col bg-sidebar border-l border-sidebar-border overflow-hidden"
        style={{
          background: 'linear-gradient(180deg, var(--sidebar) 0%, rgba(24,23,21,0.98) 100%)',
        }}
      >
        {/* Logo */}
        <div className="flex items-center justify-between p-4 border-b border-sidebar-border/50 flex-shrink-0">
          <AnimatePresence>
            {!sidebarCollapsed && (
              <motion.div
                initial={{ opacity: 0 }}
          ...
          <button
            ...
            className="p-2 rounded-lg hover:bg-sidebar-accent text-muted-foreground hover:text-gold transition-all flex-shrink-0"
          >
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-1 scrollbar-thin">
          {navSections.map((section, sIdx) => (
            <div key={section.title} className="mb-2">
              {sIdx === 1 && <div className="mx-3 my-2 border-t border-gold/10" />}
              <AnimatePresence>
                {!sidebarCollapsed && (
                  <motion.p
                    initial={{ opacity: 0 }}
        ...
        <div className="p-3 border-t border-sidebar-border/50 flex-shrink-0">
          {!sidebarCollapsed ? (
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gold/20 to-gold/5 border border-gold/25 flex items-center justify-center text-gold font-bold text-sm shadow-[0_0_8px_rgba(201,168,76,0.1)] flex-shrink-0">
              ...
              <button
                ...
                className="p-2 rounded-lg hover:bg-red-500/10 text-muted-foreground hover:text-red-400 transition-all flex-shrink-0"
                title="تسجيل الخروج"
              >
                <LogOut size={16} />
              </button>
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <motion.aside
            ...
            exit={{ x: '100%' }}
            transition={{ duration: 0.3, ease: [0.4, 0, 0.2, 1] }}
            className="fixed right-0 top-0 h-screen w-[280px] max-w-[85vw] z-50 flex flex-col bg-sidebar border-l border-sidebar-border lg:hidden overflow-hidden"
            style={{
              background: 'linear-gradient(180deg, var(--sidebar) 0%, rgba(24,23,21,0.98) 100%)',
            }}
          >
                      ...
                      <div key={item.path}>
                        <button
                          ...
                          className={cn(
                            ...
                            active
                              ? isPortal
                                ? 'bg-gradient-to-l from-gold/15 to-gold/5 text-gold border border-gold/25'
                                : 'bg-gold/10 text-gold border border-gold/20'
                              : isPortal
                                ? 'text-sidebar-foreground/80 hover:bg-gold/8 hover:text-gold border border-transparent'
                                : 'text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-foreground border border-transparent',
                          )}
                        >
                          <item.icon size={18} className={cn(
                            'shrink-0',
                            active ? 'text-gold' : isPortal ? 'text-gold/60' : 'text-muted-foreground'
                          )} />
                          <span className="flex-1 text-right whitespace-nowrap overflow-hidden">
                            {item.label}
                          </span>
                          {hasChildren && (
            ...
            <div className="p-3 border-t border-sidebar-border/50 flex-shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gold/20 to-gold/5 border border-gold/25 flex items-center justify-center text-gold font-bold text-sm flex-shrink-0">
                  {user?.name?.charAt(0) || 'م'}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-foreground truncate">{user?.name || 'المدير'}</p>
